...
function isItANum(str) {
    // empty variable to put phone number in
    let phoneNumber = ``

    for (let i = 0; i < str.length; i++){
        // checks if str[i] contains an integer. Checking if greater than -1 so that 0's dont come back as a falsey conditional.
        if (Number.isInteger(parseInt(str[i]))){
            phoneNumber += str[i]
        }
    }

    // Checks if the length of string is 11 digits and if it starts with a 0
