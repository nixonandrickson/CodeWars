...
function arrayOperations(a, k) {
    for (let i = 0; i < k; i++) {
        // Makes a max variable that holds the max value of the array
        let max = Math.max(...a)
        // As per the instructions the values of each index will be changed to (max - array[i])
        a = a.map(e => max - e)
    }

    return a
}
console.log(arrayOperations([-4, 0, -1, 0],2))
console.log(arrayOperations([-1],10000))