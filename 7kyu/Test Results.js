...
function testResult(array) {
    let class_average = Number(((array.reduce((acc,c) => acc + c, 0)) / array.length).toFixed(3))
    
    let dictionary = {
    h: array.filter(i => i >= 9).length, 
    a: array.filter(i => i === 7 || i === 8).length, 
    l: array.filter(i => i < 7).length
    }
    
    let marks = [class_average, dictionary]

    
    if (dictionary.h === array.length) {
        marks.push(`They did well`)
    }
