...
function sortIt(arr) {
    ...
    let count = sortedArr.reduce((acc, n) => {
        acc[n] = (acc[n] || 0) + 1
        return acc
    }, {})
    // Sorts the array by ascending element count (count[a] - count[b]) and at the same time descending element value (b - a)
    sortedArr.sort((a, b) => (count[a] - count[b]) || (b - a))
