...
function scoreTest(arr, right, omit, wrong) {
    ...
    // Adds up all the right, omitted, and wrong answers
    arr.forEach((e,i) => {
        if (e === 0) {
            rightNum++
        }else if (e === 1) {
    ...
    // Multiplies then adds and subtracts all the values
    let testResult = ((right * rightNum) + (omit * omitNum) - (wrong * wrongNum))
